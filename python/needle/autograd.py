        ...
        if isinstance(other, Tensor):
            ...
            if other.shape == self.shape:
                ...
                if len(self.shape) < len(other.shape):
                    return needle.ops.EWiseAdd()(
                            needle.ops.broadcast_to(self, other.shape), 
                            other)
                else:
                    return needle.ops.EWiseAdd()(
                            self,
                            needle.ops.broadcast_to(other, self.shape))
            ############
            # return needle.ops.EWiseAdd()(self, other)
        else:
        ...
        if isinstance(other, Tensor):
            ...
            if other.shape == self.shape:
                ...
                if len(self.shape) < len(other.shape):
                    ...
                    return needle.ops.EWiseAdd()(
                            self,
                            needle.ops.negate(needle.ops.broadcast_to(other, self.shape)))
            ############
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for nd in reverse_topo_order:
        nd_grad = sum_node_list(node_to_output_grads_list[nd])
        nd.grad = nd_grad
        if nd.op is None:
            continue

        input_grad = nd.op.gradient_as_tuple(nd_grad, nd)
        
        for i in range(len(nd.inputs)):
            if node_to_output_grads_list.__contains__(nd.inputs[i]):
                node_to_output_grads_list[nd.inputs[i]].append(input_grad[i])
            else:
                node_to_output_grads_list[nd.inputs[i]] = [input_grad[i]]
    ### END YOUR SOLUTION
