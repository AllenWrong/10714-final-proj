...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(new_shape) != prod(self.shape):
            print("new_shape", new_shape, "self.shape", self.shape)
            raise ValueError("prod(new_shape) != prod(self.shape)")
        else:
    ...
    def permute(self, new_axes):
        ...
        elif len(new_axes) == len(self.shape):
            pass
        else:
            assert len(new_axes) == 2
            axes = list(range(self.ndim))
            axes[new_axes[0]], axes[new_axes[1]] = axes[new_axes[1]], axes[new_axes[0]]
            new_axes = axes
        ...
        for ax in new_axes:
            ...
            new_strids.append(self.strides[ax])

        return self.as_strided(tuple(new_shape), tuple(new_strids))
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        for i in range(len(new_shape)):
            if self.shape[i] != 1 and new_shape[i] != self.shape[i]:
                raise ValueError(f"new_shape[{i}] should be {self.shape[i]}")
            if self.shape[i] == 1:
                idx_one.append(i)
    ...
    def __getitem__(self, idxs):
        ...
        return NDArray.make(
                shape=new_shape, strides=new_strides, device=self.device, 
    ...
    def flip(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        # compute offset and get new strides
        offset = 0
        new_strides = list(self.strides)
        
        if isinstance(axes, int):
            offset = (self.shape[axes] - 1) * prod(self.shape[axes+1:])
            new_strides[axes] *= -1

        elif isinstance(axes, tuple):
            for axis in axes:
                offset += (self.shape[axis] - 1) * prod(self.shape[axis+1:])
                new_strides[axis] *= -1
        else:
            raise ValueError(f"Unsupport type({type(axes)}) for axes ")       
        
        # make new array and compact
        return NDArray.make(
            self.shape, tuple(new_strides), 
            self.device, self._handle, offset).compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        for i, it in enumerate(axes):
            new_shape.append(self.shape[i] + it[0] + it[1])
        new_arr = full(new_shape, 0, device=self.device)

        # compute slices
        idxs = []
        for i, it in enumerate(axes):
            idxs.append(slice(it[0], it[0] + self.shape[i], 1))

        # set value
        new_arr[tuple(idxs)] = self
    ...
    def dilate(self, axes, dilation):
        ...
        Args:
            ...
            dilation (int):
        """
        self_axes = list(range(self.ndim))

        # compute new shape and create a new array
        new_shape = []
        for axis in self_axes:
            if axis in axes:
                new_shape.append(self.shape[axis] * (dilation + 1))
            else:
        ...
        for axis in self_axes:
            if axis in axes:
                idxs.append(slice(0, new_shape[axis], dilation + 1))
            else:
                idxs.append(slice(0, new_shape[axis]))
        
        new_array[tuple(idxs)] = self
        return new_array
...
def sum(array, axis, keepdims=False):
