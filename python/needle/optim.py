...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        if len(self.u) == 0:
            for p in self.params:
                self.u[p] = (1 - self.momentum) * (p.grad.data + self.weight_decay * p.data)
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        if len(self.m) == 0:
            for p in self.params:
                self.m[p] = (1 - self.beta1) * (p.grad.data + self.weight_decay * p.data)
                self.v[p] = (1 - self.beta2) * ndl.ops.power_scalar(
                                                   p.grad.data + self.weight_decay * p.data,
                                                   2)
                u_hat = self.m[p] / (1 - self.beta1**self.t)
                v_hat = self.v[p] / (1 - self.beta2**self.t)
                p.data = p.data - self.lr * u_hat / (np.power(v_hat.data, 1/2) + self.eps)
                del u_hat, v_hat
        else:
            for p in self.params:
                self.m[p] = self.beta1 * self.m[p] + (1 - self.beta1) * (p.grad.data + self.weight_decay * p.data)
                self.v[p] = self.beta2 * self.v[p] + (1 - self.beta2) * \
                                ndl.ops.power_scalar(
                                    p.grad.data + self.weight_decay * p.data,
                                    2)
