...
def kaiming_uniform(fan_in, fan_out, shape=None, nonlinearity="relu", **kwargs):
    ...
    ### BEGIN YOUR SOLUTION
    boud = math.sqrt(2) * math.sqrt(3 / fan_in)
    if shape is not None:
        return rand(*shape, low=-boud, high=boud, **kwargs)
    else:
        return rand(fan_in, fan_out, low=-boud, high=boud, **kwargs)
    ### END YOUR SOLUTION
...
def kaiming_normal(fan_in, fan_out, shape=None, nonlinearity="relu", **kwargs):
    ...
    if shape is not None:
        randn(*shape, mean=0, std=std, **kwargs)
    else:    
        return randn(fan_in, fan_out, mean=0, std=std, **kwargs)
    ### END YOUR SOLUTION
