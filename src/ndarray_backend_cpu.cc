...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  while (going) {
      ...
      while (idx_vec[last_dim] / shape.at(last_dim) != 1) {
          int32_t idx_a = 0;
          for (int i = 0; i < ndim; i++) {
              idx_a += idx_vec[i] * strides.at(i);
          }
          out->ptr[idx_out++] = a.ptr[idx_a + offset];
          idx_vec[last_dim]++;
      }
      ...
      for (int i = last_dim; i >= 0; i--) {
          ...
          if (idx_vec[0] == shape[0]) {
              going = false;
              break;
          }

          if (idx_vec[i] / shape[i] == 1) {
              idx_vec[i] = 0;
              carry = 1;
          } else {
              carry = 0;
...
void EwiseSetitem(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  while (going) {
      ...
      while (idx_vec[last_dim] / shape.at(last_dim) != 1) {
          int32_t idx_out = 0;
          for (int i = 0; i < ndim; i++) {
              idx_out += idx_vec[i] * strides.at(i);
          }
          out->ptr[idx_out + offset] = a.ptr[idx_a++];
          idx_vec[last_dim]++;
      }
...
void MatmulTiled(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m,
  ...
  for (size_t i = 0; i < m / TILE; i++) {
      for (size_t j = 0; j < p / TILE; j++) {
          float temp[tile_num];
          // Initialize temp
          for (size_t temp_idx = 0; temp_idx < tile_num; temp_idx++) {
              temp[temp_idx] = 0;
          }
          ...
          for (size_t temp_idx = 0; temp_idx < tile_num; temp_idx++) {
              out->ptr[i * p * TILE + j * TILE * TILE + temp_idx] = temp[temp_idx];
          }
