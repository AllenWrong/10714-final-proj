...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
      // compute idx_vec
      int* idx_vec = new int[shape.size];

      size_t idx = gid;
      for (int i = shape.size - 1; i >= 0; i--){
          idx_vec[i] = idx % shape.data[i];
      ...
      for (int i = 0; i < shape.size; i++) {
          idx_a += idx_vec[i] * strides.data[i];
      }
      out[gid] = a[idx_a + offset];
      delete idx_vec;
  }  
  /// END YOUR SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size,
  ...
  if (gid < size) {
      ...
      uint32_t idx_out = 0;
      for (int i = 0; i < shape.size; i++) {
          idx_out += idx_vec[i] * strides.data[i];
      }
      out[idx_out + offset] = a[gid];
      delete idx_vec;
  }
}
...
__global__ void ScalarPowKernel(const scalar_t* a, const scalar_t val, scalar_t* out, size_t size) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void ScalarMaxKernel(const scalar_t* a, const scalar_t val, scalar_t* out, size_t size) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t size) {
    ...
    if (gid < size) {
        ...
        for (int i = 0; i < reduce_size; i++) {
            val = fmaxf(val, a[gid * reduce_size + i]);
        }
        out[gid] = val;
    }
}
